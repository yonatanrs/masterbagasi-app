...
abstract class BaseBottomNavigationBarItem { }

class CustomIconBottomNavigationBarItem extends BaseBottomNavigationBarItem {
  final Widget icon;
  final CustomBottomNavigationBarItem customBottomNavigationBarItem;
  final String? tooltip;
  final VoidCallback? onTap;

  CustomIconBottomNavigationBarItem({
    required this.icon,
    required this.customBottomNavigationBarItem,
    this.tooltip,
    this.onTap
  }) : super();
}

class CustomBottomNavigationBarItemWithIconBuilder extends CustomBottomNavigationBarItem {
  CustomBottomNavigationBarItemWithIconBuilder({
...
class CustomBottomNavigationBarItem extends BaseBottomNavigationBarItem {
  final Widget icon;
...
class CustomBottomNavigationBar extends StatefulWidget {
  ...
  CustomBottomNavigationBar({
    ...
    this.landscapeLayout,
        ...
        assert(
        items.every((BaseBottomNavigationBarItem item) {
          if (item is CustomBottomNavigationBarItem) {
  ...
  final ValueChanged<CustomBottomNavigationBarSelectedIndex>? onTap;
...
class _IconUpStackBottomNavigationTile extends StatelessWidget {
  final BottomNavigationBarType type;
  ...
  late final ReverseTween<Color?> _reverseColorTween;
  final TextStyle selectedLabelStyle;
  final IconThemeData? unselectedIconTheme;
  final BottomNavigationBarLandscapeLayout layout;
  final VoidCallback? onTap;

  _IconUpStackBottomNavigationTile(
    this.child,
    this.type,
    this.item,
    this.iconSize,
  ...
  ) : _reverseColorTween = ReverseTween(colorTween);
  ...
  Widget build(BuildContext context) {
    ...
    final double topPadding = selectedFontSize / 2.0 + selectedIconDiff / 2.0;
    final double bottomPadding = selectedFontSize / 2.0 + unselectedIconDiff / 2.0;
    final double bottomNavigationBarHeight = kBottomNavigationBarHeight + additionalBottomPadding;

    Widget result = Padding(
      padding: EdgeInsets.only(
        top: selectedFontSize / 2.0 + selectedIconDiff / 2.0,
        bottom: selectedFontSize / 2.0 + unselectedIconDiff / 2.0,
      ),
      child: child
    );

    final Color? iconColor = _reverseColorTween.evaluate(animation);
        ...
        child: ConstrainedBox(
          ...
          child: Stack(
            children: [
              ...
              Align(
                alignment: Alignment.center,
                child: _StatelessTile(
                  layout: layout,
                  icon: _StatelessTileIcon(
                  ...
                  iconVisible: true,
                  label: _StatelessLabel(
                    item: item,
                    labelStyle: const TextStyle(color: Colors.white),
                  ),
                  labelVisible: item.showLabel ? (item.hideLabelWhenInactive ? selected : true) : false,
                ),
              )
            ]
          )
...
class _StatelessTile extends StatelessWidget {
  const  _StatelessTile({
    Key? key,
    required this.layout,
    required this.icon,
...
class _StatelessTileIcon extends StatelessWidget {
  const _StatelessTileIcon({
    Key? key,
...
class _StatelessLabel extends StatelessWidget {
  const _StatelessLabel({
    Key? key,
    required this.item,
...
class _CustomBottomNavigationBarState extends State<CustomBottomNavigationBar> with TickerProviderStateMixin {
  ...
  void _resetState() {
    ...
    BaseBottomNavigationBarItem currentItem = widget.items[widget.currentIndex];
    _controllers[widget.currentIndex].value = 1.0;
    if (currentItem is CustomBottomNavigationBarItem) {
      _backgroundColor = currentItem.backgroundColor;
    } else {
  ...
  void _pushCircle(int index) {
    BaseBottomNavigationBarItem baseBottomNavigationBarItem = widget.items[index];
    if (baseBottomNavigationBarItem is CustomBottomNavigationBarItem) {
      if (baseBottomNavigationBarItem.backgroundColor != null) {
        _circles.add(
          _Circle(
            state: this,
            index: index,
            color: baseBottomNavigationBarItem.backgroundColor!,
            vsync: this,
  ...
  void didUpdateWidget(CustomBottomNavigationBar oldWidget) {
    ...
    if (widget.currentIndex != oldWidget.currentIndex) {
      ...
      BaseBottomNavigationBarItem item = widget.items[widget.currentIndex];
      if (item is CustomBottomNavigationBarItem) {
        if (_backgroundColor != item.backgroundColor)
  ...
  List<Widget> _createTiles(BottomNavigationBarLandscapeLayout layout) {
    ...
    final List<List<dynamic>> itemsWithClicableIndex = <List<dynamic>>[];
    int clickableIndexTemp = 0;
    for (int i = 0; i < widget.items.length; i++) {
      BaseBottomNavigationBarItem item = widget.items[i];
      if (item is CustomBottomNavigationBarItem) {
        itemsWithClicableIndex.add(<dynamic>[widget.items[i], i, clickableIndexTemp]);
        clickableIndexTemp += 1;
      } else if (item is CustomIconBottomNavigationBarItem) {
        itemsWithClicableIndex.add(<dynamic>[widget.items[i], i, clickableIndexTemp]);
      }
    }
    final List<Widget> tiles = <Widget>[];
    for (int i = 0; i < itemsWithClicableIndex.length; i++) {
      BaseBottomNavigationBarItem item = itemsWithClicableIndex[i][0];
      if (item is CustomBottomNavigationBarItem) {
          ...
          onTap: () {
            widget.onTap?.call(
              CustomBottomNavigationBarSelectedIndex(
                currentSelectedMenuIndex: itemsWithClicableIndex[i][1],
                currentSelectedViewMenuIndex: itemsWithClicableIndex[i][2]
              ),
            );
          },
          colorTween: colorTween,
        ...
        tiles.add(_IconUpStackBottomNavigationTile(
          item.icon,
          _effectiveType,
...
class CustomBottomNavigationBarSelectedIndex {
  int currentSelectedMenuIndex;
  int currentSelectedViewMenuIndex;

  CustomBottomNavigationBarSelectedIndex({
    required this.currentSelectedMenuIndex,
    required this.currentSelectedViewMenuIndex
  });
}